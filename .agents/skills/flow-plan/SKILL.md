---
name: flow-plan
description: run the repo's standard feature or refactor workflow end to end when the user explicitly asks to use flow-plan.
---

适用场景：

- 用户明确要求使用 `flow-plan`
- 需要按仓库标准流程推进 `feature` 或 `refactor`
- 需要从分支、feature 目录和 gate 恢复开始编排整个迭代

触发边界：

- 仅在用户点名 `flow-plan` 时使用
...
- `refactor` 只改变 `feature.yaml.type`，其余流程与标准 feature 一致。

标准推进顺序：

`idea -> prd -> spec -> tasks -> impl -> test-review -> doc-release -> pr -> done`
...
- `tasks`：进入 `flow-tasks`
- `impl`：进入 `flow-impl`
- `test-review`：执行 `flow-test` 与 `flow-review`
- 若测试或 review 出现阻塞，先进入 `flow-investigate`，再按调查结论回到 `spec` 或 `impl`
- `doc-release`：进入 `flow-doc-release`
- `pr`：停止自动推进，先向用户确认“当前工作已完成，可以提交 PR”，确认后再进入 `flow-pr`

退出条件：

- feature 已进入 `pr` 等待用户确认，或已由 `flow-pr` 推进到 `done`
- 若结论为暂缓或放弃，`feature.yaml.current_gate=stopped`
